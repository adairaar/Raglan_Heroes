...
def bootstrap(df: pd.DataFrame, cols: list, n_boot: int,
    ...
    return {
        "boot_a": np.array(boot_a_list),   # (n_valid, p)
        "boot_b": np.array(boot_b_list),   # (n_valid, p)
...
def plot_parameter_ci(results: dict, orig_a: np.ndarray, orig_b: np.ndarray, out_dir: str):
    ...
    fig.suptitle("Bootstrap 95% CIs on 2PL IRT Parameters", fontweight="bold")
    plt.tight_layout()
    path = os.path.join(out_dir, "irt_bootstrap_parameter_ci.png")
...
def plot_stability_distributions(results: dict, orig_a: np.ndarray,
    ...
    fig.suptitle("Bootstrap Stability: Correlation of Parameter Vectors with Original 2PL Solution",
                 fontweight="bold")
    plt.tight_layout()
    path = os.path.join(out_dir, "irt_bootstrap_stability.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"Saved: {path}")
...
def plot_parameter_heatmap(orig_a: np.ndarray, orig_b: np.ndarray,
                            boot_a: np.ndarray, boot_b: np.ndarray, out_dir: str):
    ...
    fig.suptitle("2PL IRT Parameter Summary", fontweight="bold")
    plt.tight_layout()
    path = os.path.join(out_dir, "irt_parameter_heatmap.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"Saved: {path}")
...
def main():
    ...
    orig_df.to_csv(os.path.join(args.out, "irt_original_parameters.csv"))
    ...
    stability_df.to_csv(os.path.join(args.out, "irt_stability_summary.csv"), index=False)
    ...
    ci_df.to_csv(os.path.join(args.out, "irt_bootstrap_ci.csv"), index=False)

    # --- Plots ---
    print("\nGenerating plots...")
    plot_parameter_ci(results, orig_a, orig_b, args.out)
    plot_stability_distributions(results, orig_a, orig_b, args.out)
    plot_parameter_heatmap(orig_a, orig_b, results["boot_a"], results["boot_b"], args.out)
